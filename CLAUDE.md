...
**Run e2e tests with `just e2e <workflow>`** — `scripts/e2e.sh` handles preflight checks (Docker, gh auth, config), builds and starts the system, dispatches the workflow, waits for the conclusion, prints per-job results plus runner assignments, and tears down. Exit 0 = pass. Logs/artifacts land in `.e2e/<timestamp>-<workflow>/`. `--input k=v` (repeatable) forwards `workflow_dispatch` inputs. The harness also samples concurrent runner-container counts every 5s into `containers.csv` and reports the peak vs `max_runners` — catches reconcile over-provisioning, which no workflow assertion can see. Note `gh workflow run` uses the workflow file from the **remote** ref — workflow edits need commit+push before they take effect (the harness warns about this).
