...
**Run e2e tests with `just e2e <workflow>`** — `scripts/e2e.sh` handles preflight checks (Docker, gh auth, config), builds and starts the system, dispatches the workflow, waits for the conclusion, prints per-job results plus runner assignments, and tears down. Exit 0 = pass. Logs/artifacts land in `.e2e/<timestamp>-<workflow>/`. `--input k=v` (repeatable) forwards `workflow_dispatch` inputs. The harness also samples concurrent runner-container counts every 5s into `containers.csv` and reports the peak vs `max_runners` — catches reconcile over-provisioning, which no workflow assertion can see. On macOS the harness self-wraps in `caffeinate -i` — a host suspend mid-run freezes Docker and fakes a scaler deadlock (runners exit on stale broker sessions, jobs orphan). Note `gh workflow run` uses the workflow file from the **remote** ref — workflow edits need commit+push before they take effect (the harness warns about this).
...
Three manually-triggered workflows (`workflow_dispatch`):
...
- **`test-stress.yaml`** — Scale probe: a `prepare` job generates a dynamic mixed-profile matrix (inputs: `total_jobs` default 30, `high_cpu_count` default 3, `job_seconds`, `max_queue_minutes`) that oversubscribes `max_runners`. Jobs sleep (control-plane stress, not host stress; idle usage also keeps the adaptive adjuster at its baseline floor so exact-limit assertions stay deterministic) and only *record* results with `fail-fast: false`, so one mismatch doesn't destroy the dataset. The `gate` job aggregates limit mismatches, misroutes (job class vs `runner.name` class), non-success conclusions, and per-job queue waits from the jobs API, and exits nonzero on any violation. A failing run is a finding — see README "test-stress" for the failure-mode → signal map. Deliberately excluded from `just e2e-all`.
